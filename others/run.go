...
func CleanCache() {
	if fileExists(CACHE_FILE) {
		...
		// Remove duplicate entries in the cache, keeping the first entry
		...
		for _, line := range resultLines {
			...
			if len(fields) >= 2 {
				cachedFile := filepath.Join(TEMP_DIR, fields[1])
				if !fileExists(cachedFile) || fields[0] != getPackageName(fields[1]) {
					removeLineFromFile(CACHE_FILE, line)
				}
			}
		}
	}
